...
- В верхней панели колокольчик уведомлений периодически анимируется при наличии непрочитанных сообщений; по наведению или фокусу открывается список чатов с прямыми ссылками.
...
- `/` — приветственная страница и вход с Yandex SmartCaptcha.
- `/Identity/Account/Login` — технический POST-обработчик авторизации ASP.NET Identity; GET перенаправляет на `/`.
...
- `/ProfileEdit` — редактирование профиля и отдельная смена пароля.
- `/attachments/{publicId}` — авторизованная выдача вложения по публичному идентификатору.
- `/attachments/{publicId}/preview` — авторизованная выдача превью. Клиент проверяется по компании тикета, оператор — по назначению на тикет и разрешению `Tickets`, QA и документы базы знаний доступны администратору с разрешением `QA`. Путь из запроса не принимается.
...
| QA | `/Admin/QA` | `QA` | Ручная база вопросов/ответов, вложения, индексация и переиндексация; все отфильтрованные записи показываются без пагинации в свёрнутых по умолчанию группах «модель → серийный номер» с групповым выбором, поиск расположен отдельной строкой над сортировкой и фильтрами, а массовые действия — следующей строкой. |
...
7. Ошибки загрузки и обработки видео показываются в чате как общее сообщение «Ошибка отправки»; техническая причина сохраняется только в журнале и данных вложения.
...
| Модель | Назначение |
|---|---|
| `ApplicationUser` | Пользователь Identity, профиль доступа, клиентская организация; аватар связан с закрытым объектом через `AvatarPublicId` и провайдер хранения. |
...
| `ChatMessage` | Сообщение в тикете. Вложения имеют непредсказуемый `PublicId`, ключ объекта и провайдер `Local`/`S3`. |
...
| `KnowledgeDocument` | Загруженный документ базы знаний; файл хранится по закрытому ключу и имеет отдельный `PublicId`. |
| `KnowledgeChunk` | Унифицированный фрагмент документа, QA или завершённого чата. |
| `QAEntry` | Ручной вопрос/ответ базы знаний; вложения имеют отдельный `PublicId` и провайдер хранения. |
...
| Закрытое файловое хранилище Local/S3 | `Services/FileStorageService.cs` |
| Проверка типа, MIME, сигнатуры, размера и перекодирование изображений | `Services/FileUploadValidationService.cs` |
| Авторизованная выдача вложений | `Controllers/AttachmentsController.cs` |
| Перенос старых публичных файлов | `Services/LegacyFileStorageMigrationService.cs` |
...
- Docker собирает проект SDK .NET 10, зафиксированным через `global.json`, и запускает целевое приложение `net9.0` на runtime .NET 9.
- `.env`: реальные секреты; не коммитить и не показывать в логах.
- При локальном `dotnet run` приложение загружает `.env` из каталога решения или проекта до построения конфигурации; уже заданные системные переменные имеют приоритет. Docker-адреса `postgres`, `qdrant`, `ollama` и `libretranslate` для локального процесса преобразуются в `localhost` (PostgreSQL — порт `55432`). В Docker значения по-прежнему передаются через `env_file` без преобразования.
- Вложения по умолчанию хранятся локально в `/app/App_Data/Uploads` (`chat`, `qa`, `avatars`, `videos`), а не в `wwwroot`; запросы к старому `/uploads` возвращают 404.
- В `/Admin/Settings` выбирается `Local` или `S3`. S3-совместимое Yandex Object Storage настраивается только переменными `Storage__S3__*` из `.env`; подготовлен бакет `chatbotbucket`, но переключение запрещено до заполнения обоих ключей.
- Docker volume `app_uploads` подключён к `/app/App_Data/Uploads`. Стартовая миграция нормализует ключи в БД и переносит доступные legacy-файлы из `wwwroot/uploads`.
- Для проверки входа SmartCaptcha использует `SmartCaptcha__ServerKey` из `.env`; публичный ключ размещён только в форме входа. В Development и при обращении к `localhost`/loopback проверка капчи отключена.
