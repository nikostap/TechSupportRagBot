...
- QA без модели и серийного номера считается глобальным. QA с моделью, но без номера применяется ко всем номерам этой модели и совместимым сериям модели. Значения-заглушки номера (`Не указан`, `Н/Д`, `-`) при сохранении нормализуются в пустое поле и тоже означают «для всех номеров».
